...
def load_investor_details(investors):
    ...
    # last 5 investments
    last_5_df = df[df['investor'].str.contains(investors)].head()
    [['date', 'startup', 'vertical', 'city', 'round', 'amount']].sort_values('date', ascending=False)

    st.subheader("Most recent investments:-")
    st.dataframe(last_5_df)

    # big investments
    big_invest_series = df[df['investor'].str.contains(investors)].groupby('startup')
    ...
    yoy_series = df[df['investor'].str.contains('Sequoia Capital')].groupby('year')[
        'amount'].sum()
...
def load_startup_details(startups):
    # function to get the details of a particular startup....
    st.header(startups)
...
def load_overall_analysis():
    ...
    with col5:
        ...
        cnt_series = df.groupby('vertical').size(
    ...
    with col6:
        ...
        amnt_series = round(df.groupby('vertical')[
    ...
    city_fund_series = round(df.groupby(
        'city')['amount'].sum().sort_values(ascending=False).head(10))
    ...
    with col7:
        st.write("Startups-")
        top_su_series = round(df.groupby('startup')[
                              'amount'].sum().sort_values(ascending=False).head(10))
    ...
    with col8:
        st.write("Investors-")
        top_in_series = round(df.groupby('investor')[
                              'amount'].sum().sort_values(ascending=False).head(10))
...
if option == 'Overall Analysis':
    ...
    selected_startup = st.sidebar.selectbox(
        'Select Strartup', sorted(df['startup'].unique().tolist()))
    ...
    selected_investor = st.sidebar.selectbox(
        'Select StartUp', sorted(set(df['investor'].str.split(",").sum())))
